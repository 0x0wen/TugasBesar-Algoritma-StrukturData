...
/* Library ADT Priority Queue Char dengan representasi array secara eksplisit dan alokasi dinamik */
...
void Enqueue(PrioQueueChar *Q, infotype X)
{
    ...
    else
    {
        ...
        {
            temp[i] = Elmt(*Q, (Head(*Q) + i) % MaxEl(*Q));
        }
        ...
        for (i = 0; i < NBElmt(*Q); i++)
        {
            ...
            Elmt(*Q, (Head(*Q) + i) % MaxEl(*Q)) = temp[i];
        }
    }
}
...
void PrintPrioQueueChar(PrioQueueChar Q)
{
    ...
    for (i = 0; i < NBElmt(Q); i++)
    {
        ...
        current = Elmt(Q, (Head(Q) + i) % MaxEl(Q));
